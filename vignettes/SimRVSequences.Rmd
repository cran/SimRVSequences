...
**Please note: this portion of the vignette uses the `R` package `vcfR` to import and manipulate vcf data.  Unfortunately, this package is not available for flavor `r-patched-solaris-x86` and so interested users without access to `vcfR` will need determine a suitable alternative.  Users without access to `vcfR` can find the output from the applicable code chunks below at https://github.com/simrvprojects/1000-Genomes-Exon-Data/raw/master/Vignette%20Data.**
